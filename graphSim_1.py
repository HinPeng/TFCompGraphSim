...
class GraphSim():
  def __init__(self):
    ...
    # self.metadata_dir = "./treelstm_400_p100/"
    # self.metadata_dir = "./treelstm_4000_p100/"
    # self.metadata_dir = "./resnet50_190_p100_tf11/"
    # self.metadata_dir = "./dynamicRnn_28300_p100/"
    ...
    self.variables = ['kernel', 'bias', 'const', 'read', "beta", "grad", "batchnorm", "learing_rate", "weight"]
  ...
  def SimTensorAccess(self):
    ...
    # with open(self.metadata_dir+"tensor_access.txt", "w") as fin:
    #   for tac in tacs:
    #     fin.write(tac[0]+" "+str(tac[1])+'\n')    
    #   logging.debug("%s: %d" % (tac[0], tac[1]))
    logging.debug("record node num is : %d" % num)
    return tacs
  ...
  def access_analysisV1(self):
    ...
    for tac_ in tacs:
      ...
      if self.tensors[tensor_name].allocator_name != "GPU_0_bfc":
        # set allocator name is "GPU_0_bfc" or set "gpu_bfc" if model is "treelstm"
    ...
    if self.swap_time:
      ...
      logging.debug("mm_candidates num : %d" % len(self.mm_candidates))
      ...
      logging.info("Peak memory usage from tensor access is %f MB" % (peak_mem))
  ...
  def CheckAvaiR(self, recomp):
    ...
    for t in recomp.tensor.inputs:
      ...
      if t.name() in self.mm_decision.keys():
        # logging.error("%s's input in self.mm_decision" % recomp.name())
        return False
  ...
  def InitRecomp(self, tac_f):
    ...
    for sub_recomp in sub_recomps:
      for recomp in sub_recomp.coll.values():
        ...
        #                                         recomp.in_trigger[2]))
        # except TypeError:
        #   logging.debug("TypeError: %s" % recomp.name())
        #   continue
  ...
  def swapping_decisionTime(self, tac_f):
    ...
    for swapinfo in tac_f.values():
      ...
      if self.IsWeights(swapinfo.tensor_name):
        logging.debug("%s:is weight" % swapinfo.tensor_name)
        continue
      if self.IsSize(swapinfo.tensor_name):
        logging.debug("%s:is size" % swapinfo.tensor_name)
        continue
      ...
      if swapinfo.swap_free_time > 0:
        ...
        logging.debug("%s:swap_free_time" % swapinfo.tensor_name)
    ...
    if self.swap_ondemand:
      ...
      while True:
        ...
        if self.IsSize(swapinfo.tensor_name, 10) == False:
          s_time -= swapinfo.swap_time/4
          logging.debug("extend %s swap_time" % swapinfo.tensor_name)
        # e_time = swapinfo.swapin_info.end_time

        # n_time = swapinfo.GetFirstUseTimeAfterSwap()
        in_trigger_flag = True
        in_trigger_index = n_index
        logging.debug("%s's swap in start time: %d" % (swapinfo.tensor_name, s_time))
        while True:
          ...
          if self.tf_tensor_access[in_trigger_index][0] < s_time:
            ...
            in_trigger_flag = False
            logging.debug("%s in_trigger time : %d" % (swapinfo.tensor_name, self.tf_tensor_access[in_trigger_index][0]))
    ...
    with open(self.metadata_dir+self.innode_filename) as fin:
      ...
      while i < total_length:
        ...
        for j in range(pending_count):
          ...
          # if fanin_nodename == "PiecewiseConstant/case/Assert/AssertGuard/Assert/Switch":
          #   print node_name, t_name
      ...
      while i < total_length:
        ...
        for j in range(output_num):
          ...
          if self.tensors.__contains__(t_name):
            # logging.debug("init %s tensor info: %d" % (t_name, allocated_bytes))
            t = self.tensors[t_name]
...
# ------------------------------------------------------- #
  ...
  def getTensorMem(self):
    mem_bf_1307 = 0
    mem_af_1307 = 0
    with open(self.metadata_dir+"1_node2id.txt") as fout:
      lines = fout.readlines()
      for line in lines:
        nodename = line.split()[0]
        nodeId = int(line.split()[1])
        if nodename not in self.nodes.keys():
          if nodename not in self.error_nodes.keys():
            print "weird node: %s" % nodename
          else:
            print "error node: %s" % nodename
          continue
        if nodeId <= 1307:
          mem_bf_1307 += self.nodes[nodename].gpu_mem_allocated
        else:
          mem_af_1307 += self.nodes[nodename].gpu_mem_allocated
    print "mem_bf_1307 is %d" % mem_bf_1307
    print "mem_af_1307 is %d" % mem_af_1307
