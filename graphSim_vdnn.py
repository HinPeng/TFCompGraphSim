...
class GraphSim():
  def __init__(self):
    ...
    self.keys_filter = ["kernel", "beta", "bias", "learning_rate", "weight", 'read', 'constantfolding'] # for bert add "read"
    ...
    self.vdnn_filters = ['maxpool', 'avgpool','conv','relu']
    self.backprop_candidate_filters = ['relu_grad','conv2d_grad', 'maxpool_grad', 'avgpool_grad']
    self.backprop_filters = ['grad', 'backprop ']
      ...
      for recomp in sub_recomp.coll.values():
        ...
        if self.savemulti_tensor:
          ...
          for tt in sub_recomp.coll.values():
            if tt.nodename() in recomp.ops:
              tmp.append(tt.name())          
          fout.write("%d\t%s\t" % (len(tmp)+1, recomp.name()))
          for tn in tmp:
            fout.write("%s\t" % tn)
          # num_recompute_tensors = len(recomp.srcs_in_same_subrecomp)+1
          # fout.write("%d\t%s\t" % (num_recompute_tensors, recomp.name()))
  ...
  def swapping_decisionTime(self, tac_f):
    ...
    if self.swap_ondemand:
      ...
      while True:
        ...
        if self.IsSize(swapinfo.tensor_name, 2):
          in_trigger_name = "fxxxxxxxxxxxxxxxxxxxk"
          swapin_rc = 0
          swapin_total_rc = 0
        else:
    ...
    with open(self.metadata_dir+self.innode_filename) as fin:
      ...
      while i < total_length:
        ...
        for j in range(pending_count):
          ...
          if node_name == "v/tower_0/gradients/v/tower_0/cg/resnet_v13/Relu_grad/ReluGrad":
            logging.debug("what?: %s" % fanin_nodename)
...
# ------------------------------------------------------- #
  ...
  # For VDNN
  def IsVDNNBackpropNode(self, name):
    l_name = name.lower()
    if self.IsWeights(name):
      return False
    for filter_ in self.backprop_filters:
      if filter_ in l_name:
        return True
    return False


  def IsVDNNBackpropCandidateNode(self, name):
    l_name = name.lower()
    if self.IsWeights(name):
      return False
    for filter_ in self.backprop_candidate_filters:
      if filter_ in l_name:
        return True
    return False


  def IsVDNNConvNode(self, name):
    l_name = name.lower()
    if "conv" in l_name:
      return True
    return False
  ...
  def IsVDNNCandidateNode(self, name):
    l_name = name.lower()
    if self.IsWeights(name):
      return False
    if self.IsVDNNBackpropNode(name):
      return False
    for filter_ in self.vdnn_filters:
      if filter_ in l_name:
        return True
    return False


  def IsVDNNCandidateTensor(self, tensor):
    node_name = tensor.node_name
    if not self.IsVDNNCandidateNode(node_name):
      return False
    flag = False
    con_node = ''
    depth = -1
    for out_node in self.nodes[node_name].fanout_nodes:
      if not self.IsVDNNCandidateNode(out_node.node_name):
        logging.debug("F**K:%s" % out_node.node_name)
        continue
      if self.IsVDNNConvNode(out_node.node_name):
        # find the out node with the least depth, add candidate tensor swap out controller
        logging.debug("%s out node: %s %d" % (node_name, out_node.node_name, out_node.depth))            
        if depth < 0:
          depth = out_node.depth
          control_node = out_node
        elif depth > out_node.depth:
          depth = out_node.depth
          control_node = out_node
        elif depth == out_node.depth:
          if len(control_node.fanout_nodes) > len(out_node.fanout_nodes):
            control_node = out_node
        flag = True
    if flag:
      logging.debug("%s next conv node is : %s" % (node_name, control_node.node_name))
      for tmp in control_node.fanout_nodes:
        if tmp.node_name not in tensor.vdnn_swapout_controller: 
          tensor.vdnn_swapout_controller.append(tmp.node_name)
    return flag    
  

  def SetVDNNSwapInIntrigger(self, node, flag):
    node_name = node.node_name.lower()
    if "grad" not in node_name:
      return None
    for input_tensor in node.fanin_tensors:
      input_node_name = input_tensor.node_name
      if self.IsVDNNBackpropCandidateNode(input_node_name):
      # if self.IsVDNNBackpropNode(input_node_name):
        if flag:
          return input_node_name
        intrigger_name = self.SetVDNNSwapInIntrigger(self.nodes[input_node_name], True)
        if intrigger_name is None:
          logging.debug("1: %s" % input_node_name)
          continue
        return intrigger_name
    for input_tensor in node.fanin_tensors:
      input_node_name = input_tensor.node_name
      intrigger_name = self.SetVDNNSwapInIntrigger(self.nodes[input_node_name], flag)
      if intrigger_name is None:
        logging.debug("2: %s %d" % (input_node_name, flag))
        continue
      return intrigger_name
    return None
        
  
  def Swapping_decision_VDNN(self):
    backprop_node = []
    candidate_tensor = []
    for node in self.nodes.values():
      if self.IsWeights(node.node_name):
        logging.debug("Ignore Weight Node: %s" % node.node_name)
        continue
      if self.IsVDNNBackpropNode(node.node_name):
        backprop_node.append(node.node_name)  
    logging.info("Init VDNN %d backprop node" % len(backprop_node))    
    # get candidate tensor
    for name in backprop_node:
      logging.debug("backprop node %s" % name)
      for tensor in self.nodes[name].fanin_tensors:
        if self.IsVDNNCandidateTensor(tensor):
          logging.debug("Candidate tensor: %s" % tensor.name())             
          if tensor not in candidate_tensor:
            candidate_tensor.append(tensor)
    logging.info("Init %d VDNN candidate tensor" % len(candidate_tensor))
    # set intrigger
    fout = open(self.metadata_dir+"swap_info.log", 'w')
    swapin = dict()
    candidate_tensor = sorted(candidate_tensor, key=lambda x:x.allocated_bytes, reverse=True)
    intrigger_list = []
    num = 0
    for tensor in candidate_tensor:
      # if self.IsSize(tensor.name(), 64): # for resnet152
      if self.IsSize(tensor.name(), 2): # for inception3     
        continue
      swapin[tensor.name()] = list()
      node_name = tensor.node_name
      swapin_depth = -1
      for out_node in self.nodes[node_name].fanout_nodes:
        if out_node.node_name in backprop_node:
          input_slot = out_node.getInputSlot(tensor.name())
          swapin[tensor.name()].append((out_node.node_name, input_slot))
          if swapin_depth < 0:
            swapin_depth = out_node.depth
            swapin_node = out_node
            continue
          if swapin_depth > out_node.depth:
            swapin_depth = out_node.depth
            swapin_node = out_node
      # find intrigger
      # depth = -1
      # candidate_innode = ' '
      # for input_ in swapin_node.fanin_tensors:
      #   input_node = self.nodes[input_.node_name]
      #   if depth < 0:
      #     depth = input_node.depth
      #     candidate_innode = input_node
      #     continue
      #   if depth < input_node.depth:
      #     depth = input_node.depth
      #     candidate_innode = input_node
      # depth = -1
      # intrigger_name = None
      # for input_ in candidate_innode.fanin_tensors:
      #   intrigger = self.nodes[input_.node_name]
      #   if depth < 0:
      #     depth = intrigger.depth
      #     intrigger_name = intrigger.node_name
      #   if depth < intrigger.depth:
      #     intrigger_name = intrigger.node_name
      #     depth = intrigger.depth
      intrigger_name = self.SetVDNNSwapInIntrigger(swapin_node, False)
      if intrigger_name is None:
        logging.debug("Couldn't set intrigger for %s" % swapin_node.node_name)
        continue
      # if intrigger_name in intrigger_list:
      #   continue
      num += 1
      logging.debug("%s requested byte %.2f" % (tensor.name(), tensor.requested_bytes))
      # intrigger_list.append(intrigger_name)
      fout.write("%s\t%d\t%d\n" % (tensor.node_name, tensor.tid, len(swapin[tensor.name()])))
      for tmp in swapin[tensor.name()]:
        fout.write("%s\t%d\n" % (tmp[0], tmp[1]))
      fout.write("%d\n" % len(tensor.vdnn_swapout_controller))
      for tmp in tensor.vdnn_swapout_controller:
        fout.write("%s\n" % tmp)
      fout.write("%s\n0\n" % intrigger_name)
    fout.close()
    logging.info("Already Vdnn swap decision, Choose %d tensor" % num)
...
if __name__ == '__main__':
  ...
  graph_sim.Swapping_decision_VDNN()
  exit(0)
