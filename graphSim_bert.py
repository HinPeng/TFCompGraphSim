...
class GraphSim():
  def __init__(self):
    ...
    # self.metadata_dir = "./transformer_6144_p100/"
    ...
    self.keys_filter = ["kernel", "beta", "grad", "bias", "learning_rate", "weight"]
    # self.keys_filter = ["kernel", "beta", "grad", "bias", "learning_rate", "weight", 'read'] # for bert add "read"
    ...
    self.invalid_recomp = []
    self.recomp_depth = 1
    self.savemulti_tensor = True
    self.savemulti_tensor_num = 100
    self.log_tensor_access_list = False
    self.share_buffer_tensors = dict()
    self.access_distance = 200000000
  ...
  def access_analysisV1(self):
    ...
    peakmem = collections.OrderedDict()
    cpu_access = 0
    cpu_access_mem = 0      
    if self.swap_time:
      for swapinfo in tac.values():
        node_name = swapinfo.tensor_name[:-2]
        assert node_name in self.nodes.keys()
        dealloc_time = 0
        for fanout_node in self.nodes[node_name].fanout_nodes:
          if fanout_node.end_time > dealloc_time:
            dealloc_time = fanout_node.end_time
        swapinfo.deallocate_time = dealloc_time
        if len(swapinfo.access_list) > 1:
          swapinfo.access_list.sort(key=lambda x: x[1])
          # swapinfo.DeallocateTime() # Not accurate
          self.GetMaxAccessInterval(swapinfo)
        else:
          if len(self.nodes[swapinfo.tensor_name.split(':')[0]].fanout_nodes) > len(swapinfo.access_list):
            cpu_access += 1
            cpu_access_mem += swapinfo.allocated_bytes
            continue
          peakmem[swapinfo.tensor_name] = (swapinfo.allocated_time, swapinfo.deallocate_time)
          t_access = [(0, swapinfo.allocated_time)]
          t_access += swapinfo.access_list
          # swapinfo.access_list = t_access
          # self.GetMaxAccessInterval(swapinfo)
      logging.info("CPU:%d, mem %f" % (cpu_access, cpu_access_mem))
      all_tensors = {k:v for k,v in tac.items()}
      # self.peakmem_util.InitFromSwapInfo(self.mm_candidates.values())
      self.peakmem_util.InitFromSwapInfo(all_tensors.values())
      peak_memory = self.peakmem_util.GetPeakMemory()
      peakmem_ = sorted(peakmem.items(), key=lambda x: x[1][1]-x[1][0], reverse=True)
      # peakmem.sort(key=lambda x: x[1][1]-x[1][0], reverse=True)
      one_total_mem = 0
      one_total_num = 0
      count_ = 0
      tmp = [v for k,v in tac.items() if len(v.access_list) > 1]
      for k,v in peakmem_:
        if v[0] >= self.peakmem_util.right_peak_time:
          continue
        if v[1] <= self.peakmem_util.left_peak_time:
          continue
        logging.debug("%s: mem: %f, %d, %d" % (k, tac[k].allocated_bytes, v[0], v[1]))
        one_total_num += 1
        one_total_mem += tac[k].allocated_bytes
        # add create time in access list
        # if self.IsWeights(k):
        #   logging.info("%s is weight, %d, %d" % (k, v[0], v[1]))
        if v[1]-v[0] < self.access_distance:
          continue
        t_access = [(-1, tac[k].allocated_time)]
        t_access += tac[k].access_list
        tac[k].access_list = t_access
        self.GetMaxAccessInterval(tac[k])
        count_ += 1
      logging.info("Access once candidate number: %d, total memory: %f MB" % (one_total_num, one_total_mem))
      logging.info("ADD Access once tensor %d to mem candidates %d " % (count_, len(tmp)))
      self.nouse_mem = self.GetUselessMemory()
      logging.info("No use memory: %f" % self.nouse_mem)

      if self.mem_limit == 0:
        self.required_saving = 0
      else:
        self.required_saving = peak_memory - self.mem_limit

      logging.info("Peak memory usage from tensor access is %f MB" % (peak_memory+self.nouse_mem))
      ...
      if self.mem_limit == 0:
        ...
        self.required_saving = 14 - self.mem_limit
  ...
  def gettime(self,
    ...
    if len(i_ac) <= 3:
      try:
        ...
        logging.debug("ERROR: input access_time > tensor create time")
        return -1
  ...
  def GetRecompSrc(self, recomp, t, c_coll, a_coll):
    ...
    if len(t.inputs) == 0:
      ...
      #   return
      if self.IsVar(t.name()):
        logging.debug("Add 3, %s for %s" % (t.name(), recomp.name()))
    ...
    for input_ in t.inputs:
      ...
      #     recomp.AddSrc((2, input_.name()))
        ...
        if i_last_ac > t_recomp_ac:
          ...
          recomp.srcs_ops.append(input_.node_name)
  ...
  def EvaluateRP(self, recomp, tac_f, flag=False):
    def rp_src_cmp(x, y):
      if x[1][0] == y[1][0]:
        if x[1][1] == y[1][1]:
          return 0
    ...
    tmp = dict()
    if flag:
      for name in recomp.inputs:
        node_name = name[:-2]
        try:
          assert not tmp.__contains__(name)
        except AssertionError:
          logging.debug("%s inputs: %s" % (t_name, str(recomp.inputs)))
          raise AssertionError
    ...
    if len(ttmp) == 0:
      if len(self.recomp_colle.root_) == 1:
        ttmp = [(self.recomp_colle.root_[0].name(), (1,0))]
      else:
        logging.error("???:%s" % recomp.name())
        logging.debug("%s" % str(recomp.inputs))
        return -1
    k = ttmp[0][0]
    v = ttmp[0][1]
    ...
    if v[0] != 1:
      ...
      i_name = k
      # return -1
      # logging.info("%s: cpu input: %s" % (t_name, k))
      # logging.info("%s" % str(tac_f[k].access_list))
      # for bert
      # return 1
      # for k,v in ttmp:
    ...
    # use tensor create time to evaluate tensor compute time
    tc_time = recomp.tensor.create_time
    eva_flag = True
    eva_time = self.gettime(tc_time, i_ac, flag=eva_flag)
    if eva_time == -1:
      logging.error("Can not evaluate time for %s" % t_name)
      logging.debug("create_time:%d, %d, access_list:%s" % (tc_time, recomp.tensor.allocated_time, str(t_ac)))
      logging.debug("input: %s, create_time:%d, %d, access_list: %s" % (i_name, self.tensors[i_name].create_time, self.tensors[i_name].allocated_time, str(i_ac)))
      return -1
    # recomp.eva_time = eva_time
    else:
      if eva_flag:
        logging.debug("%s: last evaluate time, %d" % (t_name, eva_time))
        recomp.SetLastEvaTime(eva_time)
      else:
        # logging.debug("%s: evaluate time: %d, old time: %d" % (t_name, eva_time, eva_time_old))
  ...
  def InitRecomp(self, tac_f):
    ...
    for swapinfo in tac_f.values():
      ...
      # if self.IsVar(swapinfo.tensor_name):
        ...
        if len(swapinfo.access_list) == 2 and swapinfo.access_list[0][0] == -1:
          logging.debug("Ignore weights: %s" % swapinfo.tensor_name)
        else:
          continue
    ...
    for k, v in self.recomp_colle.collection.items():
      ...
      if v.IsRoot():
        if len(self.recomp_colle.root_) == 0:
          ...
          logging.debug("Got multiple root: %s" % k)
    ...
    # for recomp in recomps_:
      ...
      if attention_flag:
        prev_ = prev[0]
        if not prev_.IsCouldMerge(recomp):
          logging.debug("Couldn't merge subrecomp %s and %s" % (prev_.root_.name(), recomp.name()))
          prev_.UpdateTmpEvaTime()            
          continue
        if not prev_.IsCouldMerge(recomp):
          logging.debug("Couldn't merge subrecomp %s and %s" % (prev_.root_.name(), recomp.name())) 
          prev_.UpdateTmpEvaTime()            
          continue
        prev_.MergeRecomp(recomp)
      else:
        ...
        if len(prev) != 0 and len(succ) != 0:
          ...
          if not prev_.IsCouldMerge(recomp):
            ...
            prev_.UpdateTmpEvaTime()            
            continue
          if not succ_.IsCouldMerge(recomp):
            logging.debug("Couldn't merge subrecomp %s and %s" % (succ_.root_.name(), recomp.name())) 
            succ_.UpdateTmpEvaTime()            
            continue
          prev_.MergeSucc(recomp)
          for recomp_ in succ_.coll.values():
            prev_.MergeSucc(recomp_)
            prev_.MergePrev(recomp_)
      ...
      # logging.debug("%s evatime is %d" % (recomp.name(), recomp.eva_time))
      self.mm_decision[recomp.name()] = 1
      total += recomp.tensor.gpu_mem_allocated
      if required_saving != None:
        ...
        if required_saving <= 0:
          ...
          break
    error_out = open(self.metadata_dir+"error_recomp.log", 'w')
    for e_recomp in error_recomp:
      new_subrp = recomp_info.SubReComp(e_recomp)
      sub_recomps.append(new_subrp)
      e_recomp.SetTrigger(tac_f, self, r_peak_time, on_demand=self.recomp_ondemand)
      total += e_recomp.tensor.gpu_mem_allocated
      logging.debug("ERROR: Choose %s, bytes: %d MB, metric: %f" % (recomp.name(), recomp.alloc_bytes, recomp.metric))
      logging.debug("ERROR: in_trigger info: %s" % str(recomp.in_trigger))
      logging.debug("ERROR: %s swap start: %d access_list: %s" % (recomp.name(), tac_f[recomp.name()].swap_start, str(tac_f[recomp.name()].access_list)))
      error_out.write("%s\t%d\t%d\t%s\t%d\t%d\n" % (e_recomp.name(),
                                                 e_recomp.out_trigger[0],
                                                 e_recomp.out_trigger[1],
                                                 e_recomp.in_trigger[0],
                                                 e_recomp.in_trigger[1],
                                                 e_recomp.in_trigger[2]))
    error_out.close()

    logging.info("choose mmeory: %.2f" % total)
    for sub_recomp in sub_recomps:
      ...
      for recomp in sub_recomp.coll.values():
        # logging.debug("%s eva time is %d" % (recomp.name(), recomp.eva_time))
        # self.EvaluateRP(recomp, tac_f, True)
        # if recomp.eva_time > 8000:
        #   logging.debug("%s recompute time too long" % recomp.name())
        #   continue
        if tac_f[recomp.name()].access_list[0][0] != -1:
          continue
    ...
    #     logging.debug("%s, ops: %s" % (recomp.name(), str(recomp.ops)))
        ...
        # sslot = recomp.name()[-1]
        # recomp_name = node_name+':'+sslot
        # logging.debug("choose: %s" % recomp.name())
        # assert self.tensors.__contains__(recomp.name())
        # tensor = self.tensors[recomp.name()]
        # for inp in tensor.inputs:
        #   logging.debug("%s" % inp.name())
        # for src in recomp.srcs:
        #   logging.debug("%s" % src[1])
        # for input_ in recomp.inputs:
        #   logging.debug("%s" % input_)
  ...
  def swapping_decisionTime(self, tac_f):
    ...
    if self.log_tensor_access_list:
      f_access_list = open(self.metadata_dir+'access_list.log', 'w')    
    for swapinfo in tac_f.values():
      if self.IsWeights(swapinfo.tensor_name):
        continue
        if swapinfo.access_list[0][0] == -1:
          logging.debug("Ignore weights: %s" % swapinfo.tensor_name)        
        else:
          continue
      if self.IsSize(swapinfo.tensor_name):
      ...
      if swapinfo.tensor_name not in self.peakmem_util.peakmem_tensors_collec:
        ...
        if swapinfo.swap_free_time < 0:        
          continue
        if self.IsSize(swapinfo.tensor_name):
          mini_tensor += swapinfo.allocated_bytes
          logging.debug("%s not in peak memory usage time: %f, %f, %d" % (swapinfo.tensor_name, swapinfo.allocated_bytes, swapinfo.swap_free_time, swapinfo.max_access_interval))
          logging.debug("%s:access list %s" % (swapinfo.tensor_name, str(swapinfo.access_list)))
      ...
      if swapinfo.swap_free_time > 0:
        ...
        logging.debug("%s:swap_free_time is : %d, swap time: %f" % (swapinfo.tensor_name, swapinfo.swap_free_time, swapinfo.swap_time))
        logging.debug("%s:swap start %d, access list %s" % (swapinfo.tensor_name, swapinfo.swap_start, str(swapinfo.access_list)))
      if self.log_tensor_access_list:
    ...
    logging.debug("Mini Tensors memory: %f" % mini_tensor)
    ...
    if self.swap_ondemand:
      ...
      access_once_tensor = []
      for swapinfo_ in mm_left_queue:
        if swapinfo_.access_list[0][0] == -1:
          access_once_tensor.append(swapinfo_)
      sorted(access_once_tensor, key=lambda x:x.access_list[1][1]-x.access_list[1][0], reverse=True)
      for t in access_once_tensor:
      ...
      while True:
        ...
        if required_saving != None:
          ...
          if required_saving <= 0:
            ...
            break
      logging.info("Total Swap memory: %f" % total_swap_size)
      ...
      while True:
        ...
        # s_time = swapinfo.swapin_info.start_time - swapinfo.swap_time/3
        s_time = swapinfo.swapin_info.start_time
        while True:
          # TODO: check if this in_trigger to early that in peak memory
          in_trigger_index -= 1
          if self.tf_tensor_access[in_trigger_index][0] < r_peak_time:
          # if in_trigger_index == swapinfo.access_list[swapinfo.swap_start][0]:
      ...
      while i < total_length:
        ...
        for j in range(pending_count):
          ...
          if not self.tensors.__contains__(t_name):
            ...
            t.InitCreateTime(self.nodes[fanin_nodename].end_time)
...
# ------------------------------------------------------- #
  ...
  def InitNodesDepth(self):
    root_node = []
    # Init root node depth 0
    for node in self.nodes.values():
      if len(node.fanin_tensors) == 0:
        node.depth = 0
        root_node.append(node.node_name)
    logging.info("root node num: %d" % len(root_node))
    for node_name in self.nodes.keys():
      if node_name not in root_node:
        node = self.nodes[node_name]
        node.updateNodeDepth(self.nodes)
    nodes = sorted(self.nodes.values(), key=lambda x:x.depth, reverse=False)
    # for node in nodes:
    #   logging.debug("%s depth %d" % (node.node_name, node.depth))     
      # logging.debug("%s" % str([out.node_name for out in node.fanout_nodes]))
    
  def UpdateSwapDecision(self):
    nodes_seq = sorted(self.nodes.values(), key=lambda x: x.depth, reverse=True)
    tacs = []
    while True:
      if len(nodes_seq) == 0:
        break
      ...
      if not node.gpu_time:
        ...
        continue
      for fanin_tensor in node.fanin_tensors:
        if fanin_tensor.name() == "bert/encoder/layer_0/output/LayerNorm/batchnorm/mul:0":
          logging.debug("FANIN: %s" % node.node_name)
        tacs.append((fanin_tensor.name(), node.depth))
    tac = dict()
    for tac_ in tacs:
      tensor_name = tac_[0]
      access_depth = tac_[1]
      ...
      if not tac.__contains__(tensor_name):
          tac[tensor_name] = []
      tac[tensor_name].append(access_depth)
    swap_policy = list()
    with open(self.metadata_dir+self.swapping_log, 'r') as fout:
      swap_policy = [policy.strip('\n').split() for policy in fout.readlines()]
    for policy in swap_policy:
      tensor = policy[0]
      access_list = tac[tensor]
      if len(access_list) == int(policy[1]):
        logging.debug("NODIFF:%s" % (tensor))        
        continue
      elif len(access_list) < int(policy[1]):
        logging.debug("%s new access_list %d, old %d" % (tensor, len(access_list), int(policy[1])))
        continue
      max_access_interval = access_list[1]-access_list[0]
      swapout_index = 0
      for i in range(2, len(access_list)):
        if max_access_interval < access_list[i]-access_list[i-1]:
          max_access_interval = access_list[i]-access_list[i-1]
          swapout_index = i-1
      swapout_rc = len(access_list) - swapout_index - 1
      in_trigger = policy[3]
      policy[1] = len(access_list)
      policy[2] = swapout_rc  
      if in_trigger not in tac.keys():
        continue
      if len(tac[in_trigger]) > int(policy[4]):
        swapin_depth = access_list[swapout_rc+1]
        in_trigger_depth = 0
        distance = -1
        for access_depth in tac[in_trigger]:
          if access_depth < swapin_depth:
            if distance > swapin_depth-access_depth or depth == -1:
              distance = swapin_depth-access_depth
              in_trigger_depth = access_depth
        swapin_rc = len(tac[in_trigger]) - tac[in_trigger].index(in_trigger_depth) - 1
      policy[4] = len(tac[in_trigger])
      policy[5] = swapin_rc
    with open(self.metadata_dir+self.swapping_log, 'w') as fin:
      for policy in swap_policy:
        fin.write("%s\t%d\t%d\t%s\t%d\t%d\n" % (policy[0],\
          int(policy[1]), int(policy[2]), policy[3], int(policy[4]), int(policy[5])))


  def InitBertTensorInfo(self):
    with open(self.metadata_dir+'tensor_info.txt', 'r') as fin:
      for line in fin.readlines():
        line = line.strip('\n').split('\t')
        node_name = line[1].split(':')[0]
        node_depth = -1
        if self.nodes.__contains__(node_name):
          node_depth = self.nodes[node_name].depth
        if not self.share_buffer_tensors.__contains__(line[2]):
          self.share_buffer_tensors[line[2]] = list()
          self.share_buffer_tensors[line[2]].append([line[1], node_depth])
        else:
          if line[1] not in self.share_buffer_tensors[line[2]]:
            self.share_buffer_tensors[line[2]].append([line[1], node_depth])
    for tensor_buffer in self.share_buffer_tensors:
      tensor_info = self.share_buffer_tensors[tensor_buffer]
      sorted(tensor_info, key=lambda x:x[1])
      flag = True
      base = -1
      for item in tensor_info:
        if flag:
          base = item[1]
        else:      
          logging.debug("%s: %s" % (tensor_buffer, str(tensor_info)))
...
if __name__ == '__main__':
  ...
  # for bert
  # update swap decision
  graph_sim.InitNodesDepth()
  # graph_sim.UpdateSwapDecision()
  # exit(0)
  # graph_sim.InitBertTensorInfo()
  # exit(0)
