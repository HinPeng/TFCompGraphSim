...
class SubReComp():
  ...
  def IsCouldMerge(self, recomp):
    for input_ in recomp.inputs:
      if input_ in self.coll.keys():
        recomp.tmp_eva_time = self.coll[input_].tmp_eva_time + recomp.tmp_eva_time
        logging.debug("%s change input %s, evatime %d" % (recomp.name(), input_, recomp.tmp_eva_time))
        if recomp.tmp_eva_time > self.rptime_limit:
          return False
    for rp in self.coll.values():
      if recomp.name() in rp.inputs:
        rp.tmp_eva_time = rp.tmp_eva_time+recomp.tmp_eva_time
        logging.debug("%s change input %s, evatime %d" % (rp.name(), recomp.name(), rp.tmp_eva_time))                      
        if rp.tmp_eva_time > self.rptime_limit:
          return False
    return True


  def UpdateTmpEvaTime(self):
    for rp in self.coll.values():
      rp.tmp_eva_time = rp.eva_time
  ...
  def MergeSucc(self, recomp):
    ...
    for input_ in replace_inputs:
      ...
      recomp.srcs_ops.append(input_.split(":")[0])
      recomp.srcs_ops += self.coll[input_].srcs_ops
      recomp.AddSrcsInSameSubRecomp(input_)
      recomp.eva_time += self.coll[input_].eva_time
      logging.debug("update eva time: %s, %d" % (recomp.name(), recomp.eva_time))
    recomp.inputs = list(set(recomp.inputs))
    recomp.srcs_ops = list(set(recomp.srcs_ops))
  ...
  def MergePrev(self, recomp):
    ...
    for rp in self.coll.values():
      if recomp.name() in rp.inputs:
        ...
        rp.srcs_ops.append(recomp.tensor.node_name)
        rp.srcs_ops += recomp.srcs_ops        
        rp.AddSrcsInSameSubRecomp(recomp.name())
        rp.eva_time += recomp.eva_time
        logging.debug("update eva time: %s, %d" % (rp.name(), rp.eva_time))
        rp.inputs = list(set(rp.inputs))
        rp.srcs_ops = list(set(rp.srcs_ops))
  ...
  def InitRPConnection(self):
    ...
    for recomp in left_queue:
      ...
      if len(recomp.srcs_mask) == 0:
        recomp.rank=0
        logging.debug("%s input's type is 1, we set its rank=0" % recomp.name())
        tmp.append(recomp)
    curr_queue += tmp
    for rp in tmp:
...
class ReComp():
  def __init__(self,
    ...
    self.ops = []
    # self.srcs_ops = [tensor.node_name]
    self.srcs_ops = []
    ...
    self.eva_time = -1
    self.last_eva_time = -1
    self.tmp_eva_time = -1
    self.metric = 0

    self.sub_rp = None
    # srcs in one subrecomp
    self.srcs_in_same_subrecomp = []
  ...
  def InitInputs(self):
    self.inputs = [src[1] for src in self.srcs]
  ...
  def IsRoot(self):
    ...
    if len(self.srcs) == 0:
      logging.debug("%s has no inputs" % self.name())
      return True
  ...
  def SaveSrcsInfo(self, fin):
    fin.write("%s\t%d\t%d\n" % (self.name(), len(self.srcs), len(self.srcs_ops)))
    for src in self.srcs:
      fin.write("%d\t%s\n" % (src[0], src[1])) # (type, src_name)
    for src_op in self.srcs_ops:
      fin.write("%s\n" % src_op)
  
  def SaveRankAndEvaTime(self, fin):
    fin.write("%s\t%d\t%f\n" % (self.name(), self.rank, self.eva_time))
  ...
  #     else:
  def AddSrcsInSameSubRecomp(self, tensor_name):
    self.srcs_in_same_subrecomp.append(tensor_name)