...
# ------------------------------------------------------- #
  ...
  def InitBertTensorInfo(self):
    share_buffer_tensors = dict()
    with open(self.metadata_dir+'tensor_info.txt', 'r') as fin:
      for line in fin.readlines():
        line = line.strip('\n').split('\t')
        if len(line[2]) == 1:
          continue
        if not share_buffer_tensors.__contains__(line[2]):
          share_buffer_tensors[line[2]].append(line[1])
        else:
          if line[1] not in share_buffer_tensors[line[2]]:
            share_buffer_tensors.append(line[1])
...
class GraphSim():
      ...
      with open(self.metadata_dir+"tensor_access.txt") as fin:
        ...
        for line in fin:
          ...
          if not self.tensors.__contains__(tensor_name):
            ...
            if not self.ngpu_tensor_access.__contains__(tensor_name):
              self.ngpu_tensor_access[tensor_name] = []
            self.ngpu_tensor_access[tensor_name].append((line_num, access_time))
            # tensor's allocator is on CPU-side
          else:
            if self.tensors[tensor_name].allocator_name != "GPU_0_bfc":
              if not self.ngpu_tensor_access.__contains__(tensor_name):
  ...
  def gettime(self,
    ...
    if len(i_ac) <= 3:
      try:
        ...
        logging.info("%s" % str(i_ac))
        logging.info("%s" % str(t_ac))
      return (t_time - i_ac[0][1])
    else:
      for i in range(len(i_ac)-1, -1, -1):
        if i_ac[i][1] > t_time:
  ...
  def EvaluateRP(self, recomp, tac_f):
    ...
    try:
      ...
      i_ac = self.ngpu_tensor_access[i_name]
      # for k, v in tac_f.items():
      #   logging.debug("%s" % k)
      # exit(1)
  ...
  def swapping_decisionTime(self, tac_f):
    ...
    for swapinfo in tac_f.values():
      ...
      # can be added then
      # if self.swap_ondemand:
      #   if swapinfo.allocated_bytes > self.node_swap_limit:
      mm_left_queue.append(swapinfo)
    ...
    if self.swap_ondemand:
      ...
      while True:
        ...
        swapin_rc = 0
        swapin_total_rc = 0
        logging.debug("%s, %d" % (swapinfo.tensor_name, swapinfo.allocated_bytes))
        total_swap_size += swapinfo.allocated_bytes
        fout.write("%s\t%d\t%d\t%s\t%d\t%d\n" % (swapinfo.tensor_name,
                                                 swapout_total_rc,
                                                 swapout_total_rc-swapout_rc,
      ...
      while True:
        ...
        if in_trigger_name in tac_f.keys():
          ...
          access_indicies = [v for v,_ in self.ngpu_tensor_access[in_trigger_name]]
          assert (in_trigger_index in access_indicies)
          swapin_rc = len(access_indicies) - access_indicies.index(in_trigger_index) - 1
          swapin_total_rc = len(access_indicies)
        else:
